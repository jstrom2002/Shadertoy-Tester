...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
	float v = 0.0;
	vec2 in_coord = fragCoord;
	in_coord = ((in_coord * 2.0) + 1.0) * iResolution.xy;

	// Visualize 1D, 2D, and 3D
	if (in_coord.y > iResolution.y / 2.0) {
		if (in_coord.x < iResolution.x / 2.0) {
			// 1D
			float coord = in_coord.x * 0.05 + iTime * 5.0 - 10.0;
			float height = NOISE(coord) * iResolution.y / 2.0;
			v = clamp((height - in_coord.y + iResolution.y / 2.0) / (iResolution.y * 0.02), 0.0, 1.0);
		}
		else if (in_coord.x < iResolution.x / 2.0 + 1.0) {
			fragColor.rgb = vec3(1.0);
			return;
		}
		else {
			// 2D
			vec2 coord = in_coord.xy * 0.1 - vec2(iTime * 5.0, iResolution.y / 2.0);
			v = NOISE(coord);
		}
	}
	else if (in_coord.y > iResolution.y / 2.0 - 1.0) {
		fragColor.rgb = vec3(1.0);
		return;
	}
	else {
		...
		vec3 w = normalize(vec3(in_coord.xy - iResolution.xy / vec2(2.0, 4.0),
	...
	fragColor.rgb = pow(v, 0.35) * 1.3 * normalize(vec3(0.5, in_coord.xy / iResolution.xy)) + vec3(v * 0.25);
}///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
